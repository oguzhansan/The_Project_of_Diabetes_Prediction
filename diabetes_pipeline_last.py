...
def diabetes_data_prep(dataframe):
    ...
    dataframe.loc[(dataframe['AGE'] >= 35) & (dataframe['AGE'] <= 55), "NEW_AGE_CAT"] = 'mature'
    dataframe.loc[(dataframe['AGE'] > 55), "NEW_AGE_CAT"] = 'old age'
    ...
    # BloodPressure
    dataframe.loc[(dataframe['BLOODPRESSURE'] < 70), 'BLOOD_CAT'] = "low"
    dataframe.loc[(dataframe['BLOODPRESSURE'] >= 70) & (dataframe['BLOODPRESSURE'] < 90), 'BLOOD_CAT'] = "normal"
    dataframe.loc[(dataframe['BLOODPRESSURE'] >= 90), 'BLOOD_CAT'] = "high"


    # Create the New Glucose Category (According to WHO)
    dataframe.loc[(dataframe['GLUCOSE'] < 70), 'GLUCOSE_CAT'] = "hypoglycemia"
    dataframe.loc[(dataframe['GLUCOSE'] >= 70) & (dataframe['GLUCOSE'] < 100), 'GLUCOSE_CAT'] = "normal"
    dataframe.loc[(dataframe['GLUCOSE'] >= 100) & (dataframe['GLUCOSE'] < 126), 'GLUCOSE_CAT'] = "over glucose"
    dataframe.loc[(dataframe['GLUCOSE'] >= 126), 'GLUCOSE_CAT'] = "hyperglycemia"


    # # Create a Pregnancies Categorical variable
    dataframe.loc[(dataframe['PREGNANCIES'] == 0), 'PREG_CAT'] = "unpregnant"
    dataframe.loc[(dataframe['PREGNANCIES'] > 0) & (dataframe['PREGNANCIES'] <= 5), 'PREG_CAT'] = "normal"
    dataframe.loc[(dataframe['PREGNANCIES'] > 5) & (dataframe['PREGNANCIES'] <= 10), 'PREG_CAT'] = "high"
    dataframe.loc[(dataframe['PREGNANCIES'] > 10), 'PREG_CAT'] = "very high"
...
def main():
    df = pd.read_csv("/Users/oguzhansan/Projeler/PyCharm/Working Station/Diabetes/diabetes.csv")
